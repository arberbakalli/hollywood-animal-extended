...
## 12. Filtering options must not remove the control needed to choose them

**What broke.** In Compatibility Numbers, typing `heroical` into the Finale
category search hid the selector row even though the user had not selected a
Finale yet.

**The real cause.** The category search filtered both dropdown options and the
whole `.select-row` based on the current selected/default option text. For an
empty selector, that meant the row could disappear before the matching option
could be opened and chosen.

**The rule.** Search may narrow choices, but it must keep the control visible
unless the user is explicitly searching selected rows. For this app, category
search filters option visibility, not selector-row visibility.

**Cheap check for next time.** In Playwright, type a partial option name into an
unselected category search, then assert the search input, selector row, and the
matching option all remain visible.

---

## 13. Product overlap is a bug, not just a naming issue

**What broke.** Compatibility Numbers and Colman Graves were presented as two
different evaluation products even though they use the same underlying script
compatibility, score, audience, and conflict data. Colman Graves was the richer
version, while Compatibility Numbers mostly duplicated it with a few nicer UI
details.

**The real cause.** A "senior product owner" review focused on implementation
quality and feature completeness, but failed to ask whether two adjacent
features solved the same user job. That left the user to discover the product
duplication manually.

**The rule.** Product evaluation must include an overlap pass: for every feature,
state the user job, inputs, outputs, and source data. If two features share the
same job and data, recommend merging them before adding polish.

**Cheap check for next time.** Build a one-row-per-feature map with columns for
job, inputs, outputs, formulas/data, and next action: keep, merge, rename, or
delete.
